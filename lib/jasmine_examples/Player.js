class Player {
    add(str) {
        if (str.startsWith("//")) {
            ...
            const filteredArray = this.filterNumsArray(numbersArr)
            const sum = this.calculateNumbers(filteredArray);
            return sum;
        } else {
            let delimiter = '\n';
            ...
            const filteredArray = this.filterNumsArray(numbersArray)
            const sum = this.calculateNumbers(filteredArray);
            return sum;
        }
    ...
    filterNumsArray(arr) {
        return arr.filter(ele => ele < 1000)
    }
}
